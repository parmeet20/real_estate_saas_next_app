...
import { Loader2, PhoneCall, Trash2Icon, Users } from "lucide-react";
...
const Page = () => {
  ...
  const bookPropertyHandler = async (propertyId: string, userId: string) => {
    try {
      setLoading(true);
      await axios.put(
        `${API_URL}/api/properties/book/${propertyId}/${userId}`,
        null,
        {
          headers: {
      ...
      toast({
        title: "SUCCESS",
        description: "Listing Booked Successfully",
      });
    } catch (error) {
      toast({
        title: "FAILURE",
        description: "Error Booking listing",
      });
      console.log(error);
    } finally {
      setLoading(false);
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center">
        ...
        <div className="flex gap-2 mt-4 lg:mt-0">
          ...
          <Button
            onClick={() => bookmarkProperty(property?.id || "", user?.id || "")}
            size="icon"
          >
            <TooltipProvider>
              <Tooltip>
  ...
  {user?.email !== property?.owner.email && (
    <>
      {property?.bookedBy.some(bookedUser => bookedUser.id === user?.id) ? (
        <Button className="bg-green-400 text-slate-700" disabled>{"You're all set! Property booked."}</Button>
      ) : (
        <Button
          className="w-full sm:w-auto"
          disabled={loading}
          onClick={() => bookPropertyHandler(property?.id || "", user?.id || "")}
        >
          {!loading ? (
            "Book Now"
          ) : (
          ...
          <CardContent>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <Card className="hover:bg-muted p-4 rounded-lg shadow-sm flex items-center gap-2">
                ...
                <div>
                  <p className="font-semibold text-gray-700 w-[200px] dark:text-slate-300">
                    Utilities:
                  </p>
